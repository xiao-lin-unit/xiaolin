...
4. 执行`hexo d`或者`hexo deploy`推送到`gitee`
   ...
   成功之后会生成访问地址
> `gitee`的`Gitee Pages`功能已关闭，可以迁移到`github`上
### 使用`github`部署

1. 在`github`上新建仓库,仓库名称必须为`xxx.github.io`, 

2. 在`github`上生成一个`access_token`，需要注意添加哪些权限

3. 将仓库地址配置到博客根目录的`_config.yml`中, 如下

   ![部署配置](博客搭建/github-dploy-config.png)

   > 注意：
   >
   > 网站的访问路径与仓库名称和用户名称都有关系
   >
   > 以`https://{access_token}@github.com/xiao-lin-unit/xiao-lin-unit.github.io.git`为例
   >
   > 1. 解析路径为`https://{access_token}@github.com/{username}/{repository_name}.git`其中`access_token`是验证`token`，`username`是`github`中的用户名，`repository_name`是仓库名
   > 2. 仓库名以`{username}.github.io`命名，则访问地址为`https://{username}.github.io`
   > 3. 如果未按照提示2中的方式命名，则访问路径会发生变化，具体访问路径可以从仓库的`Settings -> Page`s中查看。如，我以`example.github.io`命名，则访问路径变成了`https://{username}.github.io/example.github.io`

   ![github访问地址](博客搭建/github-address.png)

4. 安装`git`部署插件`npm install hexo-deployer-git --save`和`markdown`渲染插件`npm install hexo-renderer-marked --save`

5. 执行`hexo g`或者`hexo generate`命令生成静态文件

   在`public`目录下可以看到生成的静态文件(将`public`中的内容放到`nginx`下, 做好相关配置, 也可以实现)

6. 执行`hexo d`或者`hexo deploy`推送到`github`

   此时发现`github`仓库中已经添加了内容, 这些内容是`public`目录下的静态文件

   ![码云部署](博客搭建/gitee-deploy.png)

   成功之后会生成访问地址
> 非一键部署方式可通过<a href="https://hexo.io/zh-cn/docs/github-pages">Hexo 官网</a>学习
...
5. 如果不想直接发布博客，可以创建草稿
   ...
   1. 使用`hexo new draft title -p xxx/yyy.md`会在`_drafts`目录下创建一个`xxx`目录，并创建一个`yyy.md`的博客文件
   2. `scaffolds`目录下有博客模板，可以自行修改模板内容
   3. 使用`hexo publish title`来发布博客
